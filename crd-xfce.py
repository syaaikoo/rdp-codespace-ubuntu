...
NAMA_APLIKASI = "rdp-xfce4-codespace"
...
def buat_venv(venv_dir: Path) -> None:
    ...
    subprocess.run([str(py), "-m", "pip", "install", "--upgrade", "-q", "pip", "setuptools", "wheel"],
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False) #fungsi ini kalian wajib bngt harus pakai python 3
...
def atur_logging() -> Path:
    ...
    file_log = direktori_log / "xfce-crd.log"
    logging.basicConfig(
        level=logging.INFO,
...
def pasang_desktop() -> None:
    ...
    apt(["install", "xfce4", "desktop-base", "xfce4-terminal"], "Pasang DE XFCE4")
    cmd = 'echo "exec /etc/X11/Xsession /usr/bin/xfce4-session" > /etc/chrome-remote-desktop-session'
    jalankan_perintah(["bash", "-lc", cmd], "Mengatur sesi CRD")
...
def banner() -> None:
    tabel = Table(title="Installing chrome-remote-desktop", show_lines=False, header_style="bold")
...
def main() -> None:
    try:
        ...
        wajib_arch_amd64()
        wajib_root()
        # Periksa semua perintah utama sebelum lanjut
        cek_perintah([
            "id", "useradd", "usermod", "chpasswd", "chsh", "visudo",
            "apt-get", "dpkg", "wget", "bash", "systemctl", "su", "service"
        ])
